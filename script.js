...
const cardObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            const delay = isMobile ? 0 : (entry.target.dataset.delay || 0);
            if (delay === 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Observe and set delays for cards (reduced delay on mobile)
    const cards = document.querySelectorAll('.glass-card, .project-card, .stat-card');
    const cardDelay = isMobile ? 0 : 100;
    cards.forEach((card, index) => {
        card.dataset.delay = index * cardDelay;
        cardObserver.observe(card);
    });
    
    // Observe skill items with delays
    const skillItems = document.querySelectorAll('.skill-item');
    const skillDelay = isMobile ? 0 : 50;
    skillItems.forEach((item, index) => {
        item.dataset.delay = index * skillDelay;
...
// Throttled counter animation
window.addEventListener('scroll', throttle(animateCounters, 200), { passive: true });
...
if (!isMobile && 'performance' in window) {
    window.addEventListener('load', () => {
        ...
        if (perfData) {
            console.log('⚡ Load Time:', Math.round(perfData.loadEventEnd) + 'ms');
        }
    });
}
